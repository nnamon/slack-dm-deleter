...
def main():
    # Who are you?
    iden_r = requests.get("https://slack.com/api/auth.test?token=%s&pretty=1" %
                          api_token)
    iden = json.loads(iden_r.text)

    # Get list of users
    users_r = requests.get("https://slack.com/api/users.list?token=%s&pretty=1"
                           % api_token)
    ...
    # Get list of ims
    ims_r = requests.get("https://slack.com/api/im.list?token=%s&pretty=1" %
                         api_token)
    ...
    hist_r = requests.get("https://slack.com/api/im.history?token=%s&"
                          "channel=%s&count=1000&pretty=1" %
                          (api_token, chat["id"]))
    ...
    for i in ours:
        ...
        requests.get("https://slack.com/api/chat.delete?token=%s&ts=%s&"
                     "channel=%s&pretty=1" %
                     (api_token, i["ts"], chat["id"]))
